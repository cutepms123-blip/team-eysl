/* TEAM EYSL v190 — event visual refresh + status button consistency */
(()=>{
 if(window.__EYSL_UI_V190__)return;window.__EYSL_UI_V190__=true;
...
`;document.head.appendChild(css);
 ...
 function fixMedalRankingCopy(){const text='2026 메달왕은 2026년에 열린 대회 메달만 집계해요. 역대 메달 누적은 전체 연도를 합산해요. 개인전·핀수영대회·단체전을 모두 포함하고, 단체전은 해당 출전 선수에게 각각 1개로 집계해요. 순위는 금메달 수 → 은메달 수 → 동메달 수 순이며 모두 같으면 공동순위예요. 확인되지 않은 2026 수원 연맹회장배 은메달 1건은 현재 집계에서 제외했어요.';document.querySelectorAll('#teamEventRankings .card.meta').forEach(el=>{const t=(el.textContent||'').trim();if((t.includes('메달 수')||t.includes('단체전은 해당 출전 선수')||t.includes('순위의 팀원을 누르면'))&&t!==text)el.textContent=text})}
