/* TEAM EYSL notice date poll calendar v119 */
(()=>{
  if(window.__NOTICE_DATE_PICKER_V119__)return;
  window.__NOTICE_DATE_PICKER_V119__=true;
  ...
  style.id='NOTICE_DATE_PICKER_V119_STYLE';
  style.textContent=`
    ...
    .pollCalendarOverlayV119{position:fixed;inset:0;z-index:2147483600;background:rgba(0,0,0,.42);display:flex;align-items:flex-end;justify-content:center}
    ...
    .pollCalWeekV119,.pollCalGridV119{display:grid;grid-template-columns:repeat(7,1fr);text-align:center}.pollCalWeekV119{font-size:12px;color:#555;margin-bottom:10px}.pollCalGridV119{row-gap:7px}.pollCalDayV119{aspect-ratio:1/1;max-width:48px;width:100%;justify-self:center;border:0;border-radius:50%;background:transparent;font-size:17px;color:#111;display:grid;place-items:center;padding:0}.pollCalDayV119.other{color:#c9c9c9}.pollCalDayV119.selected{background:#1479e8;color:#fff;font-weight:850}.pollCalDayV119.today:not(.selected){color:#1479e8;font-weight:850}
  ...
  function enhance(){
    ...
    m.classList.toggle('pollDateModeV119',dateMode());
    const old=m.querySelector('#pollDatePickerBtnV119');
    if(!dateMode()){if(old)old.remove();return}
    ...
    m.querySelectorAll('#pollV115Options input[type=date]').forEach(i=>{i.readOnly=true;i.setAttribute('aria-label','선택된 투표 날짜')});
  }
  ...
  function openCalendar(){
    ...
    let overlay=document.getElementById('pollCalendarOverlayV119');
    if(!overlay){overlay=document.createElement('div');overlay.id='pollCalendarOverlayV119';overlay.className='pollCalendarOverlayV119';document.body.appendChild(overlay)}
    overlay.innerHTML='<div class="pollCalendarSheetV119" role="dialog" aria-modal="true"></div>';
    overlay.onclick=e=>{if(e.target===overlay)closeCalendar()};
    renderCalendar();
  }
  ...
  function applyDates(){
    ...
    while(rows.length>values.length&&rows.length>2){
  ...
  if(typeof baseSetType==='function'){
    window.pollV115SetType=function(type){
      ...
      setTimeout(()=>{enhance();if(type==='date')openCalendar()},0);
      return r;
    };
  }
