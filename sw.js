const VERSION='team-eysl-final94-notice-time';
const CACHE=`team-eysl-${VERSION}`;
const PRECACHE=['/manifest.webmanifest','/icon-192.png','/icon-512.png','/apple-touch-icon.png','/enhancements-v93.js','/timestamp-v94.js'];
...
async function navigationResponse(req){
  try{
    ...
    if(!html.includes('/enhancements-v93.js'))html=html.replace('</body>','<script src="/enhancements-v93.js?v=final94"></script></body>');
    if(!html.includes('/timestamp-v94.js'))html=html.replace('</body>','<script src="/timestamp-v94.js?v=final94"></script></body>');
