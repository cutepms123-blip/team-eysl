const VERSION='team-eysl-final105-attendance-save';
...
async function navigationResponse(req){
  try{
    ...
    if(!html.includes('/timestamp-v94.js'))html=html.replace('</body>','<script src="/timestamp-v94.js?v=final105"></script></body>');
    if(!html.includes('/join-date-v96.js'))html=html.replace('</body>','<script src="/join-date-v96.js?v=final105"></script></body>');
    if(!html.includes('/activity-comments-v98.js'))html=html.replace('</body>','<script src="/activity-comments-v98.js?v=final105"></script></body>');
    if(!html.includes('/application-order-v99.js'))html=html.replace('</body>','<script src="/application-order-v99.js?v=final105"></script></body>');
    if(!html.includes('/manual-roster-v100.js'))html=html.replace('</body>','<script src="/manual-roster-v100.js?v=final105"></script></body>');
    if(!html.includes('/attendance-save-v105.js'))html=html.replace('</body>','<script src="/attendance-save-v105.js?v=final105"></script></body>');
