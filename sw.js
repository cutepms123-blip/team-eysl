const VERSION='team-eysl-final99-application-order';
const CACHE=`team-eysl-${VERSION}`;
const PRECACHE=['/manifest.webmanifest','/icon-192.png','/icon-512.png','/apple-touch-icon.png','/timestamp-v94.js','/join-date-v96.js','/activity-comments-v98.js','/application-order-v99.js'];
...
async function navigationResponse(req){
  try{
    ...
    if(!html.includes('/timestamp-v94.js'))html=html.replace('</body>','<script src="/timestamp-v94.js?v=final99"></script></body>');
    if(!html.includes('/join-date-v96.js'))html=html.replace('</body>','<script src="/join-date-v96.js?v=final99"></script></body>');
    if(!html.includes('/activity-comments-v98.js'))html=html.replace('</body>','<script src="/activity-comments-v98.js?v=final99"></script></body>');
    if(!html.includes('/application-order-v99.js'))html=html.replace('</body>','<script src="/application-order-v99.js?v=final99"></script></body>');
