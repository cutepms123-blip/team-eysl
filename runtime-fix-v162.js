/* TEAM EYSL v188 — visible runtime fixes + canonical event hub icons */
(()=>{
 if(window.__EYSL_RUNTIME_V188__)return;window.__EYSL_RUNTIME_V188__=true;
 ...
 const icons=['🏊','⏰','⚡','🎉','🏆','🏅','🏁'];
 function decorateEvents(){document.querySelectorAll('#teamEvents .eventHubBtn').forEach((b,i)=>{b.querySelectorAll(':scope > .eysl-event-icon').forEach(x=>x.remove());let s=b.querySelector(':scope > .eyslV162Icon');if(!s){s=document.createElement('i');s.className='eyslV162Icon';b.prepend(s)}s.textContent=icons[i]||'🏁'})}
